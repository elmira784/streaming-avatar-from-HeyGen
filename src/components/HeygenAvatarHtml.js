    ...
    <script>
        class HeyGenAvatarClient {
            ...
            async createAndSendAnswer() {
                ...
                try {
                    // Get backend URL from session data or use default
            ...
            async speakText(text) {
                if (!this.isConnected || !this.sessionData) {
                    console.error('Avatar not connected or session data missing');
                    return;
                }
                
                console.log('SPEAK TEXT CALLED WITH:', text);
                
                try {
                    this.updateStatus('Making avatar speak: ' + text.substring(0, 50) + '...');
                    
                    const backendUrl = this.sessionData.backendUrl || 'http://localhost:4001';
                    console.log('Sending speak request to:', backendUrl + '/api/heygen/speak');
                    ...
                    this.updateStatus('Avatar is speaking: "' + text + '"');
        ...
        // Global avatar client instance
        let avatarClient = new HeyGenAvatarClient();
        ...
        window.addEventListener('message', async (event) => {
            try {
                ...
                switch (data.type) {
                    case 'initialize':
                        console.log('Initializing with session data:', data.sessionData);
                        await avatarClient.initializeSession(data.sessionData);
                        break;
                    case 'speak':
                        console.log('RECEIVED SPEAK COMMAND:', data.text);
                        console.log('Avatar client connected?', avatarClient.isConnected);
                        console.log('Session data available?', !!avatarClient.sessionData);
